...
object MonoAstPrinter {
  ...
  /**
    ...
    */
  private def printHandlerRule(rule: MonoAst.HandlerRule): (ast.Symbol.OpSym, List[DocAst.Expr.AscriptionTpe], DocAst.Expr) = rule match {
  ...
  /**
    * Returns the [[DocAst]] representation of `rule`.
    */
  private def printCatchRule(rule: MonoAst.CatchRule): (ast.Symbol.VarSym, Class[?], DocAst.Expr) = rule match {
