...
object TypedAstPrinter {
  ...
  /**
    ...
    */
  private def printHandlerRule(rule: TypedAst.HandlerRule): (ast.Symbol.OpSym, List[DocAst.Expr.AscriptionTpe], DocAst.Expr) = rule match {
  ...
  /**
    * Returns the [[DocAst]] representation of `rule`.
    */
  private def printCatchRule(rule: TypedAst.CatchRule): (ast.Symbol.VarSym, Class[?], DocAst.Expr) = rule match {
